...
export default function Board({ estados, items, render, camposTransicion, onMover }) {
  ...
  return (
    <div>
      ...
      <BoardDnd
        ...
        render={render} camposTransicion={camposTransicion} onMover={onMover}
      />
    </div>
  );
}

function BoardDnd({ vista, estados, items, render, camposTransicion, onMover }) {
        ...
        <div className="kanban">
          {estados.map((est) => (
            <Columna key={est.id} estado={est}
              items={items.filter((i) => i.estado === est.id)} render={render} />
          ))}
        </div>
      ) : (
        <ListaView estados={estados} items={items} render={render} />
      )}
...
function Columna({ estado, items, render }) {
  ...
  return (
    <div ref={setNodeRef} className={'kcol' + (isOver ? ' drop-hover' : '')}>
      ...
      <div className="kcol-body">
        {items.map((it) => <Tarjeta key={it.id} item={it} render={render} />)}
      </div>
    </div>
  );
}

function Tarjeta({ item, render }) {
...
function ListaView({ estados, items, render }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      {estados.map((est) => {
        const propios = items.filter((i) => i.estado === est.id);
        return (
          <ListaGrupo key={est.id} estado={est} items={propios} render={render} />
        );
      })}
    </div>
  );
...
function ListaGrupo({ estado, items, render }) {
  const { setNodeRef, isOver } = useDroppable({ id: estado.id });
  return (
    <div ref={setNodeRef}>
      ...
      <div className="card" style={{ borderColor: isOver ? 'var(--brand)' : undefined }}>
        {items.length === 0 ? (
          ...
          <ListaFila key={it.id} item={it} render={render} />
        ))}
      </div>
    </div>
  );
...
function ListaFila({ item, render }) {
  const { attributes, listeners, setNodeRef, isDragging } = useDraggable({ id: String(item.id) });
  return (
    <div ref={setNodeRef} {...listeners} {...attributes}
      style={{ padding: '11px 16px', borderBottom: '1px solid var(--line-2)', cursor: 'grab', opacity: isDragging ? .5 : 1 }}>
      {render(item)}
    </div>
  );
}
